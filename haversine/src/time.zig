...
pub const Trace = struct {
    ...
    pub fn begin(comptime src: std.builtin.SourceLocation) Trace {
        return .{
            .name = null,
            .function = src.fn_name,
            .file = src.file,
            .line = src.line,
            .ts = getRealWorldCPUTime(),
        };
    }

    pub fn beginNamed(comptime src: std.builtin.SourceLocation, name: []const u8) Trace {
        return .{
            .name = name,
            .function = src.fn_name,
            .file = src.file,
    ...
    pub fn end(self: Self) void {
        const e = getRealWorldCPUTime();
        if (self.name) |n| {
            print("[{s}]: {s}:{s}:{} took {d} secs\n\n", .{ n, self.file, self.function, self.line, e - self.ts });
        } else {
            print("{s}:{s}:{} took {d} secs\n\n", .{ self.file, self.function, self.line, e - self.ts });
        }
    }
};
